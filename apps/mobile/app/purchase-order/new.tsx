...
// list — needed so the order can actually reference per-store stock. Cleared the moment
  ...
  return (
    <View style={[sf.screen, { paddingTop: insets.top }]}>
      <View style={sf.appBar}>
        ...
        <Text style={sf.appBarTitle}>Add Items to Order</Text>
...
export default function NewPurchaseOrderScreen() {
  ...
  // Prefill every field from the order being edited. The handoff store (stashed by
  // whichever screen navigated here) avoids a redundant fetch; a deep link or app
  // restart falls back to a direct lookup by id — same pattern as sale/new.tsx.
  useEffect(() => {
    ...
    load.then(async (txn) => {
      if (!txn) { Alert.alert("Not found", "Could not load this purchase order."); router.back(); return; }
      ...
      if (paidNow > 0) { setIsPaid(true); setPaidAmt(String(parsedNotes.paidAmt ?? paidNow)); }

      setLoadingEdit(false);
    }).catch(() => {
      Alert.alert("Error", "Could not load this purchase order.");
      router.back();
    });
  // Deliberately NOT depending on `catalog`/`parties`/`selectedCompanyId` — this must run
  // exactly once per editId. `takeHandoffTxn` is consume-once, so a re-run would fall
  // through to a from-scratch reload that wipes out any edits already made.
  ...
  async function handleSave(saveAndNew = false) {
    ...
    if (!selectedCompanyId) {
      Alert.alert("Select a Company", "Pick a specific company from the switcher before saving a purchase order.");
      return;
    }
    setSaving(true);
    try {
      const party = parties.find((p) => p.id === supplierId) ?? parties.find((p) => p.name === supplier) ?? selectedParty;
      ...
      if (isEdit && params.editId) {
        ...
        await api.createTransaction({ ...payload, type: "purchase_order", number: billNo });
        if (saveAndNew) { router.replace("/purchase-order/new"); }
        else { router.back(); }
      }
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[s.screen, { paddingTop: insets.top }]}>
        ...
        <View style={s.appBar}>
          ...
          <Text style={s.appBarTitle}>Purchase Order</Text>
          <TouchableOpacity hitSlop={8}>
        ...
        <ScrollView keyboardShouldPersistTaps="always" contentContainerStyle={s.scroll}>
          ...
          <View style={[s.outlinedField, supplierFocused && s.outlinedFieldFocused]}>
            <Text style={[s.floatLabel, supplierFocused && s.floatLabelFocused]}>Supplier *</Text>
            <TextInput
          ...
          {items.length > 0 && (
            <View style={s.billedCard}>
              <View style={s.billedHeader}>
                ...
                <Text style={s.billedHeaderTxt}>Ordered Items</Text>
              </View>
              {items.map((item, idx) => (
        ...
        <View style={[s.footer, { paddingBottom: insets.bottom + 8 }]}>
          ...
          <TouchableOpacity style={s.saveBtn} onPress={() => handleSave(false)} disabled={saving || loadingEdit}>
            <Text style={s.saveTxt}>{saving ? "Saving…" : loadingEdit ? "Loading…" : isEdit ? "Update" : "Save"}</Text>
          </TouchableOpacity>
