  ...
  return (
    <View style={[sf.screen, { paddingTop: insets.top }]}>
      <View style={sf.appBar}>
        ...
        <Text style={sf.appBarTitle}>Add Items to Estimate/Quotation</Text>
  ...
  async function handleSave(saveAndNew = false) {
    ...
    try {
      ...
      await api.createTransaction({
        ...
        total,
        balance: total,
        notes: JSON.stringify({ items, discountPct, discountRs, note }),
      });
      if (saveAndNew) { router.replace("/estimate/new"); }
      else { router.back(); }
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[s.screen, { paddingTop: insets.top }]}>
        ...
        <View style={s.appBar}>
          ...
          <Text style={s.appBarTitle}>Estimate/Quotation</Text>
          <TouchableOpacity hitSlop={8}>
        ...
        <ScrollView keyboardShouldPersistTaps="always" contentContainerStyle={s.scroll}>
          ...
          {/* Party Balance row (above customer field, shown when selected) */}
          {selectedParty && (
            <View style={s.partyBalanceRow}>
              <View style={{ flex: 1 }} />
              <Text style={s.partyBalanceLbl}>Party Balance:  </Text>
              <Text style={s.partyBalanceVal}>
                Rs {Math.abs(selectedParty.balance ?? 0).toFixed(4)}
              </Text>
            </View>
          )}
          ...
          {/* Tax, Discount & Charges — flat style */}
          ...
          <View style={s.descWrap}>
            <View style={s.descBox}>
              <Text style={s.descFloatLabel}>Description</Text>
              <TextInput
                style={s.descInput}
                value={note}
...
const s = StyleSheet.create({
  ...
  descWrap: {
    flexDirection: "row", gap: 10, padding: 14, backgroundColor: "#fff",
  },
  descBox: {
    flex: 1, borderWidth: 1, borderColor: colors.textMuted,
    borderRadius: 6, padding: 10, minHeight: 80,
  },
  descFloatLabel: { fontSize: 11, color: colors.textMuted, marginBottom: 4 },
