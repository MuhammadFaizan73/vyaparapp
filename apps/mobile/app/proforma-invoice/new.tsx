  ...
  return (
    <View style={[sf.screen, { paddingTop: insets.top }]}>
      <View style={sf.appBar}>
        ...
        <Text style={sf.appBarTitle}>Add Items to Proforma Invoice</Text>
...
/* ══════════════════════════════════════════════════════
   MAIN PROFORMA INVOICE FORM
══════════════════════════════════════════════════════ */
export default function NewProformaInvoiceScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { parties } = useParties();
  const { items: catalog } = useItems();

  const [refNo] = useState("1");
  ...
  async function handleSave(saveAndNew = false) {
    ...
    try {
      ...
      if (saveAndNew) { router.replace("/proforma-invoice/new"); }
      else { router.back(); }
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[s.screen, { paddingTop: insets.top }]}>
        ...
        <View style={s.appBar}>
          ...
          <Text style={s.appBarTitle}>Proforma Invoice</Text>
          <TouchableOpacity hitSlop={8}>
            <Ionicons name="settings-outline" size={22} color={colors.textLight} />
          </TouchableOpacity>
        </View>
        ...
        <View style={s.refRow}>
          <View style={s.refCell}>
            ...
            <TouchableOpacity style={s.refValRow}>
              <Text style={s.refVal}>{refNo}</Text>
        ...
        <ScrollView keyboardShouldPersistTaps="always" contentContainerStyle={s.scroll}>
          ...
          {/* Party Balance (shown when party selected) */}
          {selectedParty && selectedParty.balance != null && (
            <View style={s.partyBalanceBar}>
              <Text style={s.partyBalanceTxt}>
                Party Balance: Rs {Math.abs(selectedParty.balance).toFixed(4)}
                {selectedParty.balance > 0 ? "  (they owe you)" : selectedParty.balance < 0 ? "  (you owe them)" : ""}
              </Text>
            </View>
          )}
          ...
          <View style={[s.outlinedField, customerFocused && s.outlinedFieldFocused]}>
            ...
            <TextInput
              ...
              value={customer}
              onChangeText={(t) => { setCustomer(t); setCustomerId(null); setSelectedParty(null); setShowPartyDrop(true); }}
            ...
            <View style={s.partyDrop}>
              {filteredParties.slice(0, 8).map((p) => (
                <TouchableOpacity key={p.id} style={s.partyRow}
                  onPress={() => { setCustomer(p.name); setCustomerId(p.id); setSelectedParty(p); setShowPartyDrop(false); }}>
...
const s = StyleSheet.create({
  ...
  partyBalanceTxt: { fontSize: 12.5, color: "#1d4ed8", fontWeight: "500" },
  ...
  billedSummary: { paddingHorizontal: 14, paddingVertical: 10, backgroundColor: "#f9fafb" },
