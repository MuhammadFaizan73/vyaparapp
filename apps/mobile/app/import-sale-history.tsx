...
// a blank row before the real header — scan for the first row containing every required
// header instead of hardcoding an index. Ported from ImportSaleHistoryPage.tsx.
...
function buildSummary(saleRows: RawRow[], itemRows: RawRow[]): Summary {
  ...
  return {
    totalInvoiceRows: saleRows.length, totalItemRows: itemRows.length,
...
export default function ImportSaleHistoryScreen() {
  ...
  async function handlePickFile() {
    ...
    try {
      ...
      const { rows: saleRows, headerRowIndex: saleIdx } = readSheetRows(wb.Sheets[saleSheetName]!, REQUIRED_SALE_REPORT_HEADERS);
      const { rows: itemRows, headerRowIndex: itemIdx } = readSheetRows(wb.Sheets[itemSheetName]!, REQUIRED_ITEM_DETAILS_HEADERS);
      if (saleIdx === -1 || itemIdx === -1) {
        const parts: string[] = [];
        if (saleIdx === -1) parts.push(`Couldn't find the expected header row in "${SALE_REPORT_SHEET}"`);
      ...
      setParseError(err?.message ?? `Couldn't read this file. Make sure it's a valid .xls or .xlsx file with "${SALE_REPORT_SHEET}" and "${ITEM_DETAILS_SHEET}" tabs.`);
    } finally {
      setPicking(false);
    }
  }
  ...
  return (
    <View style={[s.screen, { paddingTop: insets.top }]}>
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>Import Sale History</Text>
      </View>

      {stage === "upload" && (
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
          <View style={s.guideCard}>
            <Text style={s.guideTitle}>📋  Export your old sale register</Text>
            <Text style={s.guideText}>Needs two tabs: a "{SALE_REPORT_SHEET}" tab (one row per invoice) and an "{ITEM_DETAILS_SHEET}" tab (one row per item sold, linked by invoice number).</Text>
            <Text style={[s.guideText, { fontWeight: "700" }]}>{SALE_REPORT_SHEET} needs:</Text>
            <View style={s.chipsRow}>{REQUIRED_SALE_REPORT_HEADERS.map((h) => <Text key={h} style={s.chip}>{h}</Text>)}</View>
      ...
      {stage === "preview" && summary && (
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
          ...
          <View style={s.chipsRow}>
            ...
            <Text style={s.chip}>{summary.parties.length} parties</Text>
            <Text style={s.chip}>{summary.invoices.length} invoices</Text>
          </View>
          {(summary.skippedInvoices > 0 || summary.skippedItemRows > 0) && (
            <View style={s.errorBanner}>
              <Text style={s.errorTxt}>
                {summary.skippedInvoices > 0 && `${summary.skippedInvoices} invoice(s) skipped (missing/duplicate data). `}
          ...
          <View style={s.previewCard}>
            ...
            {summary.invoices.length > 100 && <Text style={s.moreTxt}>+ {summary.invoices.length - 100} more invoices</Text>}
          </View>
          <Text style={s.footNote}>Items, Parties, and Sale invoices are created automatically. Re-running this same file is safe — already-imported invoices are skipped.</Text>
          <View style={s.footerRow}>
            ...
            <TouchableOpacity style={s.primaryBtn} onPress={() => void startImport()}>
              <Text style={s.primaryTxt}>Import {summary.invoices.length} Invoices</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      )}
        ...
        <View style={s.centerBox}>
          {progress.status === "error" ? (
            ...
            <>
              ...
              <Text style={s.doneTitle}>{progress.invoicesImported} invoices imported</Text>
              <Text style={s.footNote}>{progress.itemsCreated} items created · {progress.partiesCreated} parties created{progress.invoicesSkipped > 0 ? ` · ${progress.invoicesSkipped} skipped` : ""}</Text>
            </>
          )}
